document.addEventListener("DOMContentLoaded", function () {
	...
	// 부트스트랩 모달 객체 생성
    const submitModal = new bootstrap.Modal(document.getElementById('workSubmitModal')); 
    ...
    function validate() {
        ...
        const workDetailsNo = document.querySelector("select[name='workDetailsNo']").value;
        const workContent = document.getElementById("workContent").value.trim();

		// if 문
        if (!jobNo) {
            document.getElementById("error-jobNo").textContent = "일감을 선택해주세요!";
            isValid = false;
        }
        if (!workDate) {
            document.getElementById("error-workDate").textContent = "작업일자를 입력해주세요!";
            isValid = false;
        }
        if (!workTime || workTime <= 0) {
            document.getElementById("error-workTime").textContent = "근무 시간을 정확히 입력해주세요!";
            isValid = false;
        }
        if (!workDetailsNo) {
            document.getElementById("error-workDetailsNo").textContent = "작업분류를 선택해주세요!";
            isValid = false;
        }
        if (!workContent) {
            document.getElementById("error-workContent").textContent = "상세 설명을 입력해주세요!";
            isValid = false;
        }
